...
class Refine:
    ...
    def refine_plan(self, plan_file, planner):
        ...
        """
        c_st = self.helper.get_initial_state()
        dct={}
        action_list = []
        with open(ROOT_PATH+"/objects.json") as json_file:
            try:
                
                ob = json.load(json_file)
                for tup,dat in ob.items():
                    for tup1,dat1 in dat.items():
                        for tupT2,dat2 in dat1.items():
                            if tup2=="load_loc":
                                dct[str(T1)+"_iloc"]=dat2
               
            except (ValueError, KeyError, TypeError):
                print "JSON error"
        
        with open(ROOT_PATH+"/problem.pddl.soln") as text:
            try:
                file_tuple = text.readlines()
                print "1"
                file_tuple = file_tuple[:-1]
                for j in file_tuple:
                    print j
                    j = j[1:-2]
                    print j
                    ...
                    if "move" in j:
                        ...
                        print wd[3]
                        print dct[w[3]]
                        path,c_st,dest = self.get_path(c_st,dct[w[3]])
                        print "5"
                        action_list.append(("move",path))
                    elif "pick" in j:
                        action_list.append(("pick",wd[1],c_st))
                        print "7"
                    else:
                        action_list.append(("place",wd[1],wd[2],c_st))
                        print "6"
                    print "4"
            except (ValueError, KeyError, TypeError):
                print "file error"
